...
Start with the [Threads overview](/threads) when you are deciding between the prebuilt Drawer, a custom headless UI, and historical import. This page is the implementation path for building the thread UI yourself.
...
<Callout type="info" title="Prebuilt thread UI: Threads Drawer">
  ...
  **[Threads Drawer](/prebuilt-components/copilot-threads-drawer)** — drop
...
- The Enterprise Intelligence Platform. For a managed project, follow [Connect an Existing App to Hosted Intelligence](/premium/existing-app-hosted-intelligence) first.
- An OpenAI API key
...
<Callout type="info" title="Migrating existing history?">
  ...
  [Import thread history](/threads-import), or jump to
  [Import ADK threads](/google-adk/threads-import) or
  [Import LangGraph threads](/langgraph-python/threads-import).
</Callout>

<Steps>
      <Step>
        ### Confirm that your Runtime uses Intelligence

        For Cloud-Hosted Enterprise Intelligence, complete [Connect an Existing App to Hosted Intelligence](/premium/existing-app-hosted-intelligence) first. Keep its `CopilotKitIntelligence`, `identifyUser`, key, hosted URLs, handler hooks, and four HTTP exports in your Runtime route. Do not replace that route with a plain `CopilotRuntime` when you add the headless UI.

        Threads use the entitlement state returned by the managed project, not a license token in the managed app. For self-hosted deployments, see [Self-Hosting Enterprise Intelligence](/premium/self-hosting) for the Helm chart install and database configuration.
